...
export default function TicketWorkbenchFormPage({ sessionState }) {
  ...
  const bootstrapState = useJson(`/api/ticket-workbench/bootstrap${toQueryString({ ticketId: id, companyId: formState?.companyId || requestedCompanyId })}`);
        ...
        {bootstrap && formState && (
          <form className="owner-form" onSubmit={submit}>
            <div className="owner-form-grid">
              ...
              <label>
                Entitlement
                <select value={formState.companyEntitlementId} onChange={event => setFormState(current => ({ ...current, companyEntitlementId: event.target.value }))}>
              ...
              <label className="form-span-2">
                External issue
                <input value={formState.externalIssueLink} onChange={event => setFormState(current => ({ ...current, externalIssueLink: event.target.value }))} placeholder="https://github.com/..." />
              </label>
            </div>
