...
export function headerNavigation(
  ...
  const navigation = Array.isArray(session?.navigation)
    ? session.navigation
    : [];
...
export function filterNavigation(
  ...
  return labels.flatMap(
    (label) => filtered.find((link) => link.label === label) || [],
  );
}

export function orderedNavigation(
  navigation: SessionNavigationLink[] | null | undefined,
  labels: string[],
): SessionNavigationLink[] {
...
export function ticketLabelForRole(
  role: Role | undefined,
...
export function isRoleTicketRoute(
  role: Role | undefined,
  pathname: string,
): boolean {
  if (role === "support") {
